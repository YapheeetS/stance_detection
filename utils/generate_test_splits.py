...
def cross_validation(full_x,full_y ,folds):
    iterations =[]
    split_size = int(len(full_y) / folds)

    for i in range(0,folds):
        X = full_x.tolist()
        Y = full_y

        test_x = X[i*split_size : (i+1)*split_size]
        test_y = Y[i*split_size : (i+1)*split_size]
        test_x = np.c_[test_x]
        test_y = np.array(test_y)
        test = (test_x,test_y)
        del X[i * split_size : (i + 1) * split_size]
        del Y[i * split_size : (i + 1) * split_size]
        #train_x= np.array(X)
        train_x= np.c_[X]
        train_y= np.array(Y)

        train = (train_x,train_y)
        iterations.append((train,test))
